...
def extract_links_from_program_page(html_content: str) -> list[str]:
    ...
    for container in movie_containers:
        main_links = container.find_all(
            "a", class_="undlink", href=re.compile(r"^program/25/[^/]+$")
        )
        ...
        for subtitle in subtitle_containers:
            subtitle_links = subtitle.find_all(
                "a", href=re.compile(r"^program/25/[^/]+$")
            )
...
def extract_movie_details_from_page(html_content: str, url: str) -> MovieDetails:
    ...
    for div in small_divs:
        ...
        for nag_div in nag_divs:
            ...
            if not original_title and not any(
                word in text.lower() for word in ["202", "/", "napisy"]
            ):
                original_title = text
...
async def scrape_multiple_movies(
    movie_urls: list[str], base_url: str = "https://www.nowehoryzonty.pl/"
) -> list[dict]:
    """
...
def save_movie_details_to_json(
    movie_details: list[MovieDetails], filename: str = "raw_movie_details.json"
):
...
async def main():
    ...
    save_movie_details_to_json(
        movie_details, os.path.join("data", "raw_movie_details.json")
    )
